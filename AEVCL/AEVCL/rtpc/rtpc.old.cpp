...
typedef struct
{
	...
	char recoverystate; // 0、无故障，1、重连，2、执行中，-1、退出
	BOOL enable;
	RTP_HEADER* rtp; // RTP数据包
	int recvsize; // 收取大小
	BOOL recvbody; // 收取包体信号（signal）
...
void RTPC_ReceivedWork(void* state)
{
	...
	while (TRUE)
	{
		...
		for (int i = 0; i < rtpcWorkRuntime->vtWork.size(); i++) {
			...
			if (error >= 0 && work->recoverystate == 2)
			...
			if (error > 0) {
				...
				if (rtp == NULL && !work->recvbody) {
					rtp = rtp_header_parse(buffer, len);
					if (rtp != NULL && work->rtp == NULL) { // 保存一下这次的数据（以便后续处理）
						work->recvbody = TRUE;
						work->rtp = rtp;
						work->recvsize = (rtp->size - rtp->body.size);
					}
				}
				else if (work->recvbody) {
					work->recvbody = FALSE;
					work->rtp = NULL;
					work->recvsize = NULL;
					//
					memcpy(rtp->body.buf + rtp->body.size, buffer, len);
					rtp->body.size += len;
		...
		if (rtpcWorkRuntime != NULL) {
			...
			for (int i = 0; i < vt->size(); i++)
			{
				...
				if (work->recoverystate == 1) {
					...
					if (work->pack != NULL)
						delete work->pack;
					work->recoverystate = 2;
				}
				if (work->recoverystate == 2)
					sock_recovery(work->socket);
			}
		}
		sleep(500);
	}
